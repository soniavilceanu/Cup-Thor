        ...
        class Timer{
            public:
                ...
                Timer(){
                    ...
                    this -> setted = false ;
                    this -> name = "";
                    this -> helping_thread = std::thread(functie_aux);
                    
                }

                ~Timer(){
                    this -> helping_thread.join();
                }
            ...
            private:
                ...
                bool setted;
                std::string name;
                std::thread helping_thread;
                void functie_aux(){
                    while (true){
                        if (this -> is_set()){
                            if (this -> time > 0){ //Nu e gata timerul, scad timpul cu o secunda dupa ce trece o secunda in real time
                                std::this_thread::sleep_for(std::chrono::microseconds(1000));
                                this -> time = this -> time - 1;
                            }

                            else{ // e gata timerul, pun in fisier "done" si desetez
                            ...
                            //Intr-un fel e busy waiting si e un bad practice, ar trebui updatat pe viitor
